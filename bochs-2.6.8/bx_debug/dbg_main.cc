...
static FILE *debugger_dump_log = NULL; // Ken 2016-02-16
...
//******************************************Ken added 2016-02-16*****************************
void dbg_dump_printf(const char *fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  char buf[DBG_PRINTF_BUFFER_LEN+1];
  vsnprintf(buf, DBG_PRINTF_BUFFER_LEN, fmt, ap);
  va_end(ap);
  if (debugger_dump_log != NULL) {
      fprintf(debugger_dump_log,"%s", buf);
      fflush(debugger_dump_log);
    }
  //SIM->debug_puts(buf); // send to debugger, which will free buf when done.
}

void bx_dbg_print_mxcsr_state_dump(void)
{
#if BX_CPU_LEVEL >= 6
  Bit32u mxcsr = SIM->get_param_num("SSE.mxcsr", dbg_cpu_list)->get();
  ...
  dbg_dump_printf("MXCSR: 0x%05x: %s %s RC:%s %s %s %s %s %s %s %s %s %s %s %s %s %s\n", mxcsr,
     (mxcsr & (1<<17)) ? "ULE" : "ule",
     (mxcsr & (1<<15)) ? "FUZ" : "fuz",
     round_control[(mxcsr >> 13) & 3],
     (mxcsr & (1<<12)) ? "PM" : "pm",
...
void bx_dbg_print_sse_state_dump(void)
{
#if BX_CPU_LEVEL >= 6
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_SSE)) {
	  bx_dbg_print_mxcsr_state_dump();
    ...
    for(unsigned i=0;i</*BX_XMM_REGISTERS*/16;i++) {
      ...
      sprintf(param_name, "SSE.xmm%02d_0", i);
      Bit64u lo = SIM->get_param_num(param_name, dbg_cpu_list)->get64();
      dbg_dump_printf("XMM[%02u]: %08x_%08x_%08x_%08x\n", i,
         GET32H(hi), GET32L(hi), GET32H(lo), GET32L(lo));
    }
  }
  else
#endif
  {
	  dbg_dump_printf("The CPU doesn't support SSE state !\n");
  }
}

void bx_dbg_print_avx_state_dump(unsigned vlen)
{
#if BX_SUPPORT_AVX
  char param_name[20];
  ...
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_AVX)) {
    ...
    for(unsigned i=0;i<num_regs;i++) {
    	dbg_dump_printf("VMM[%02u]: ", i);
      for (int j=vlen-1;j >= 0; j--) {
        sprintf(param_name, "SSE.xmm%02u_%d", i, j*2+1);
        Bit64u hi = SIM->get_param_num(param_name, dbg_cpu_list)->get64();
        sprintf(param_name, "SSE.xmm%02u_%d", i, j*2);
        Bit64u lo = SIM->get_param_num(param_name, dbg_cpu_list)->get64();
        dbg_dump_printf("%08x_%08x_%08x_%08x", GET32H(hi), GET32L(hi), GET32H(lo), GET32L(lo));
        if (j!=0) dbg_dump_printf("_");
      }
      dbg_dump_printf("\n");
    }
  }
...
#endif
  {
	  dbg_dump_printf("The CPU doesn't support AVX state !\n");
  }
  ...
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_AVX512)) {
    for(unsigned i=0;i<8;i++) {
      sprintf(param_name, "OPMASK.k%u", i);
      Bit64u opmask = SIM->get_param_num(param_name, dbg_cpu_list)->get64();
      dbg_dump_printf("K%d: %08x_%08x\n", i, GET32H(opmask), GET32L(opmask));
    }
  }
#endif
}

void bx_dbg_print_mmx_state_dump(void)
{
#if BX_CPU_LEVEL >= 5
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_MMX)) {
    char param_name[20];
    for(unsigned i=0;i<8;i++) {
      sprintf(param_name, "FPU.st%d.fraction", i);
      Bit64u mmreg = SIM->get_param_num(param_name, dbg_cpu_list)->get64();
      dbg_dump_printf("MM[%d]: %08x_%08x\n", i, GET32H(mmreg), GET32L(mmreg));
    }
  }
  else
#endif
  {
	  dbg_dump_printf("The CPU doesn't support MMX state !\n");
  }
}


void bx_dbg_print_fpu_state_dump(void)
{
#if BX_SUPPORT_FPU
  BX_CPU(dbg_cpu)->print_state_FPU_dump(debugger_dump_log);
#else
  dbg_dump_printf("The CPU doesn't support FPU state !\n");
#endif
}

void bx_dbg_info_flags_dump(void)
{
	dbg_dump_printf("%s %s %s %s %s %s %s IOPL=%1u %s %s %s %s %s %s %s %s %s\n",
    BX_CPU(dbg_cpu)->get_ID() ? "ID" : "id",
    BX_CPU(dbg_cpu)->get_VIP() ? "VIP" : "vip",
    BX_CPU(dbg_cpu)->get_VIF() ? "VIF" : "vif",
    BX_CPU(dbg_cpu)->get_AC() ? "AC" : "ac",
...
void bx_dbg_info_registers_command_dump(int which_regs_mask)
{
  bx_address reg;

  if (which_regs_mask & BX_INFO_GENERAL_PURPOSE_REGS) {
#if BX_SUPPORT_SMP
	  dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
#endif
#if BX_SUPPORT_X86_64 == 0
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EAX);
    dbg_dump_printf("eax: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_ECX);
    dbg_dump_printf("ecx: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EDX);
    dbg_dump_printf("edx: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EBX);
    dbg_dump_printf("ebx: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_ESP);
    dbg_dump_printf("esp: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EBP);
    dbg_dump_printf("ebp: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_ESI);
    dbg_dump_printf("esi: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EDI);
    dbg_dump_printf("edi: 0x%08x %d\n", (unsigned) reg, (int) reg);
    reg = bx_dbg_get_eip();
    dbg_dump_printf("eip: 0x%08x\n", (unsigned) reg);
#else
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RAX);
    dbg_dump_printf("rax: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RCX);
    dbg_dump_printf("rcx: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RDX);
    dbg_dump_printf("rdx: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RBX);
    dbg_dump_printf("rbx: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RSP);
    dbg_dump_printf("rsp: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RBP);
    dbg_dump_printf("rbp: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RSI);
    dbg_dump_printf("rsi: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RDI);
    dbg_dump_printf("rdi: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R8);
    dbg_dump_printf("r8 : %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R9);
    dbg_dump_printf("r9 : %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R10);
    dbg_dump_printf("r10: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R11);
    dbg_dump_printf("r11: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R12);
    dbg_dump_printf("r12: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R13);
    dbg_dump_printf("r13: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R14);
    dbg_dump_printf("r14: %08x_%08x ", GET32H(reg), GET32L(reg));
    reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_R15);
    dbg_dump_printf("r15: %08x_%08x\n", GET32H(reg), GET32L(reg));
    reg = bx_dbg_get_instruction_pointer();
    dbg_dump_printf("rip: %08x_%08x\n", GET32H(reg), GET32L(reg));
#endif
    reg = BX_CPU(dbg_cpu)->read_eflags();
    dbg_dump_printf("eflags 0x%08x: ", (unsigned) reg);
    bx_dbg_info_flags_dump();
  }
#if BX_SUPPORT_FPU
  if (which_regs_mask & BX_INFO_FPU_REGS) {
	  bx_dbg_print_fpu_state_dump();
  }
#endif

  if (which_regs_mask & BX_INFO_MMX_REGS) {
    bx_dbg_print_mmx_state_dump();
  }

#if BX_SUPPORT_EVEX
  if (which_regs_mask & BX_INFO_ZMM_REGS) {
    bx_dbg_print_avx_state_dump(BX_VL512);
  }
  else
#endif
  {
#if BX_SUPPORT_AVX
    if (which_regs_mask & BX_INFO_YMM_REGS) {
     bx_dbg_print_avx_state_dump(BX_VL256);
    }
    else
#endif
    {
      if (which_regs_mask & BX_INFO_SSE_REGS) {
        bx_dbg_print_sse_state_dump();
      }
    }
  }
}

//void  bx_dbg_store_data_dump(unsigned cpu, bx_address lin, bx_phy_address phy, unsigned len, unsigned memtype, unsigned rw, Bit8u *data, Bit8u* stored_data)
//{
//
//
//
...
void bx_dbg_lin_memory_access_dump(unsigned cpu, bx_address lin, bx_phy_address phy, unsigned len, unsigned memtype, unsigned rw, Bit8u *data)
{
  bx_dbg_check_memory_watchpoints(cpu, phy, len, rw);
...
//    return;
  ...
  dbg_dump_printf("[CPU%d %s]: LIN 0x" FMT_ADDRX " PHY 0x" FMT_PHY_ADDRX " (len=%d, %s)",
     cpu,
     (rw == BX_WRITE) ? "WR" : "RD",
     lin, phy, len, get_memtype_name(memtype));
  ...
  if (len == 1) {
     Bit8u val8 = *data;
     dbg_dump_printf(": 0x%02X", (unsigned) val8);
  }
  else if (len == 2) {
     Bit16u val16 = *((Bit16u*) data);
     dbg_dump_printf(": 0x%04X", (unsigned) val16);
  }
  else if (len == 4) {
     Bit32u val32 = *((Bit32u*) data);
     dbg_dump_printf(": 0x%08X", (unsigned) val32);
  }
  else if (len == 8) {
     Bit64u data64 = * (Bit64u*)(data);
     dbg_dump_printf(": 0x%08X 0x%08X", GET32H(data64), GET32L(data64));
  }
#if BX_CPU_LEVEL >= 6
  else if (len == 16) {
     const BxPackedXmmRegister *xmmdata = (const BxPackedXmmRegister*)(data);
     dbg_dump_printf(": 0x%08X 0x%08X 0x%08X 0x%08X",
         xmmdata->xmm32u(3), xmmdata->xmm32u(2), xmmdata->xmm32u(1), xmmdata->xmm32u(0));
  }
#if BX_SUPPORT_AVX
  else if (len == 32) {
     const BxPackedYmmRegister *ymmdata = (const BxPackedYmmRegister*)(data);
     dbg_dump_printf(": 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X",
        ymmdata->ymm32u(7), ymmdata->ymm32u(6), ymmdata->ymm32u(5), ymmdata->ymm32u(4),
        ymmdata->ymm32u(3), ymmdata->ymm32u(2), ymmdata->ymm32u(1), ymmdata->ymm32u(0));
  }
#endif
     ...
     const BxPackedZmmRegister *zmmdata = (const BxPackedZmmRegister*)(data);
     dbg_dump_printf(": 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X",
        zmmdata->zmm32u(15), zmmdata->zmm32u(14), zmmdata->zmm32u(13), zmmdata->zmm32u(12),
        zmmdata->zmm32u(11), zmmdata->zmm32u(10), zmmdata->zmm32u(9),  zmmdata->zmm32u(8),
        zmmdata->zmm32u(7),  zmmdata->zmm32u(6),  zmmdata->zmm32u(5),  zmmdata->zmm32u(4),
        zmmdata->zmm32u(3),  zmmdata->zmm32u(2),  zmmdata->zmm32u(1),  zmmdata->zmm32u(0));
...
void bx_dbg_phy_memory_access_dump(unsigned cpu, bx_phy_address phy, unsigned len, unsigned memtype, unsigned rw, unsigned access, Bit8u *data)
{
  bx_dbg_check_memory_watchpoints(cpu, phy, len, rw);
...
//    return;
  ...
  dbg_dump_printf("[CPU%d %s]: PHY 0x" FMT_PHY_ADDRX " (len=%d, %s)",
     cpu,
     (rw == BX_WRITE) ? "WR" : "RD",
     phy, len, get_memtype_name(memtype));
...
void bx_dbg_disassemble_current_dump(int which_cpu, int print_time)
{
  bx_phy_address phy;
  ...
  bx_bool phy_valid = BX_CPU(which_cpu)->dbg_xlate_linear2phy(BX_CPU(which_cpu)->guard_found.laddr, &phy);
  if (! phy_valid) {
	  dbg_dump_printf("(%u).[" FMT_LL "d] ??? (physical address not available)\n", which_cpu, bx_pc_system.time_ticks());
    return;
  }
...
//    if (print_time)
//    	dbg_dump_printf("(%u).[" FMT_LL "d] ", which_cpu, bx_pc_system.time_ticks());
//    else
//    	dbg_dump_printf("(%u) ", which_cpu);

    if (BX_CPU(which_cpu)->protected_mode()) {
    	dbg_dump_printf("[0x" FMT_PHY_ADDRX "] %04x:" FMT_ADDRX " (%s): ",
        phy, BX_CPU(which_cpu)->guard_found.cs,
        BX_CPU(which_cpu)->guard_found.eip,
        bx_dbg_symbolic_address(BX_CPU(which_cpu)->cr3 >> 12,
           BX_CPU(which_cpu)->guard_found.eip,
    	...
    	dbg_dump_printf("[0x" FMT_PHY_ADDRX "] %04x:%04x (%s): ",
        phy, BX_CPU(which_cpu)->guard_found.cs,
        (unsigned) BX_CPU(which_cpu)->guard_found.eip,
        bx_dbg_symbolic_address(0,
           BX_CPU(which_cpu)->guard_found.eip & 0xffff,
...
//    dbg_dump_printf("%-25s ; ", bx_disasm_tbuf);
//    for (unsigned j=0; j<ilen; j++) {
//    	dbg_dump_printf("%02x", (unsigned) bx_disasm_ibuf[j]);
//    }
    dbg_dump_printf("\n");
  }
}

void bx_dbg_info_debug_regs_command_dump(void)
{
  bx_address dr0 = SIM->get_param_num("DR0", dbg_cpu_list)->get64();
  bx_address dr1 = SIM->get_param_num("DR1", dbg_cpu_list)->get64();
  bx_address dr2 = SIM->get_param_num("DR2", dbg_cpu_list)->get64();
  ...
  dbg_dump_printf("DR0=0x" FMT_ADDRX "\n", dr0);
  dbg_dump_printf("DR1=0x" FMT_ADDRX "\n", dr1);
  dbg_dump_printf("DR2=0x" FMT_ADDRX "\n", dr2);
  dbg_dump_printf("DR3=0x" FMT_ADDRX "\n", dr3);

  static const char *dr_ln[4] = { "Byte", "Word", "QWord", "Dword" };
  static const char *dr_rw[4] = { "Code", "DataW", "I/O", "DataRW" };

  dbg_dump_printf("DR6=0x%08x: %s %s %s %s %s %s %s\n", dr6,
      (dr6 & (1<<15)) ? "BT" : "bt",
      (dr6 & (1<<14)) ? "BS" : "bs",
      (dr6 & (1<<13)) ? "BD" : "bd",
      (dr6 & (1<<3)) ? "B3" : "b3",
  ...
  dbg_dump_printf("DR7=0x%08x: DR3=%s-%s DR2=%s-%s DR1=%s-%s DR0=%s-%s %s | %s %s | %s %s %s %s %s %s %s %s\n", dr7,
      dr_rw[(dr7 >> 28) & 3], dr_ln[(dr7 >> 30) & 3],
      dr_rw[(dr7 >> 24) & 3], dr_ln[(dr7 >> 26) & 3],
      dr_rw[(dr7 >> 20) & 3], dr_ln[(dr7 >> 22) & 3],
      dr_rw[(dr7 >> 16) & 3], dr_ln[(dr7 >> 18) & 3],
...
void bx_dbg_info_control_regs_command_dump(void)
{
  Bit32u cr0 = SIM->get_param_num("CR0", dbg_cpu_list)->get();
  bx_address cr2 = (bx_address) SIM->get_param_num("CR2", dbg_cpu_list)->get64();
  bx_phy_address cr3 = (bx_phy_address) SIM->get_param_num("CR3", dbg_cpu_list)->get64();
  dbg_dump_printf("CR0=0x%08x: %s %s %s %s %s %s %s %s %s %s %s\n", cr0,
    (cr0 & (1<<31)) ? "PG" : "pg",
    (cr0 & (1<<30)) ? "CD" : "cd",
    (cr0 & (1<<29)) ? "NW" : "nw",
    (cr0 & (1<<18)) ? "AC" : "ac",
    ...
    (cr0 & (1<<1))  ? "MP" : "mp",
    (cr0 & (1<<0))  ? "PE" : "pe");
  dbg_dump_printf("CR2=page fault laddr=0x" FMT_ADDRX "\n", cr2);
  dbg_dump_printf("CR3=0x" FMT_PHY_ADDRX "\n", cr3);
  dbg_dump_printf("    PCD=page-level cache disable=%d\n", (cr3>>4) & 1);
  dbg_dump_printf("    PWT=page-level write-through=%d\n", (cr3>>3) & 1);
#if BX_CPU_LEVEL >= 5
  Bit32u cr4 = SIM->get_param_num("CR4", dbg_cpu_list)->get();
  dbg_dump_printf("CR4=0x%08x: %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n", cr4,
    (cr4 & (1<<21)) ? "SMAP" : "smap",
    (cr4 & (1<<20)) ? "SMEP" : "smep",
    (cr4 & (1<<18)) ? "OSXSAVE" : "osxsave",
    (cr4 & (1<<17)) ? "PCID" : "pcid",
...
#if BX_SUPPORT_X86_64
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_LONG_MODE)) {
	  dbg_dump_printf("CR8: 0x%x\n", BX_CPU(dbg_cpu)->get_cr8());
  }
#endif
  Bit32u efer = SIM->get_param_num("MSR.EFER", dbg_cpu_list)->get();
  dbg_dump_printf("EFER=0x%08x: %s %s %s %s %s\n", efer,
    (efer & (1<<14)) ? "FFXSR" : "ffxsr",
    (efer & (1<<11)) ? "NXE" : "nxe",
    (efer & (1<<10)) ? "LMA" : "lma",
    (efer & (1<<8))  ? "LME" : "lme",
  ...
  if (BX_CPU(dbg_cpu)->is_cpu_extension_supported(BX_ISA_XSAVE)) {
    Bit32u xcr0 = SIM->get_param_num("XCR0", dbg_cpu_list)->get();
    dbg_dump_printf("XCR0=0x%08x: %s %s %s %s %s %s %s %s\n", xcr0,
      (xcr0 & (1<<7)) ? "HI_ZMM" : "hi_zmm",
      (xcr0 & (1<<6)) ? "ZMM_HI256" : "zmm_hi256",
      (xcr0 & (1<<5)) ? "OPMASK" : "opmask",
      (xcr0 & (1<<4)) ? "BNDCFG" : "bndcfg",
...
void bx_dbg_info_segment_regs_command_dump(void)
{
  static const char *segname[] = { "es", "cs", "ss", "ds", "fs", "gs" };
  ...
  for(int s=0;s<6;s++) {
    BX_CPU(dbg_cpu)->dbg_get_sreg(&sreg, s);
    dbg_dump_printf("%s:0x%04x, dh=0x%08x, dl=0x%08x, valid=%u\n", segname[s],
       (unsigned) sreg.sel, (unsigned) sreg.des_h,
       (unsigned) sreg.des_l, (unsigned) sreg.valid);
    if (sreg.valid) {
    	dbg_dump_printf("\t");
       bx_dbg_print_descriptor_dump(sreg.des_l, sreg.des_h);
    }
  }

  BX_CPU(dbg_cpu)->dbg_get_ldtr(&sreg);
  dbg_dump_printf("ldtr:0x%04x, dh=0x%08x, dl=0x%08x, valid=%u\n",
      (unsigned) sreg.sel, (unsigned) sreg.des_h,
      (unsigned) sreg.des_l, (unsigned) sreg.valid);

  BX_CPU(dbg_cpu)->dbg_get_tr(&sreg);
  dbg_dump_printf("tr:0x%04x, dh=0x%08x, dl=0x%08x, valid=%u\n",
      (unsigned) sreg.sel, (unsigned) sreg.des_h,
      (unsigned) sreg.des_l, (unsigned) sreg.valid);

  BX_CPU(dbg_cpu)->dbg_get_gdtr(&global_sreg);
  dbg_dump_printf("gdtr:base=0x" FMT_ADDRX ", limit=0x%x\n",
      global_sreg.base, (unsigned) global_sreg.limit);

  BX_CPU(dbg_cpu)->dbg_get_idtr(&global_sreg);
  dbg_dump_printf("idtr:base=0x" FMT_ADDRX ", limit=0x%x\n",
      global_sreg.base, (unsigned) global_sreg.limit);
}


void bx_dbg_print_descriptor_dump(Bit32u lo, Bit32u hi)
{
  Bit32u base = ((lo >> 16) & 0xffff)
             | ((hi << 16) & 0xff0000)
             | (hi & 0xff000000);
  ...
  if (s) {
    ...
    // 0=data segment, 1=code seg
    if (IS_CODE_SEGMENT(type)) {
    	dbg_dump_printf("Code segment, base=0x%08x, limit=0x%08x, %s, %s%s",
        base, g ? (limit * 4096 + 4095) : limit,
        IS_CODE_SEGMENT_READABLE(type) ? "Execute/Read" : "Execute-Only",
        IS_CODE_SEGMENT_CONFORMING(type)? "Conforming" : "Non-Conforming",
        IS_SEGMENT_ACCESSED(type)? ", Accessed" : "");
#if BX_SUPPORT_X86_64
      if (l && !d_b)
    	  dbg_dump_printf(", 64-bit\n");
      else
#endif
    	  dbg_dump_printf(", %d-bit\n", d_b ? 32 : 16);
    } else {
    dbg_dump_printf("Data segment, base=0x%08x, limit=0x%08x, %s%s%s\n",
        base, g ? (limit * 4096 + 4095) : limit,
        IS_DATA_SEGMENT_WRITEABLE(type)? "Read/Write" : "Read-Only",
        IS_DATA_SEGMENT_EXPAND_DOWN(type)? ", Expand-down" : "",
        IS_SEGMENT_ACCESSED(type)? ", Accessed" : "");
    ...
    dbg_dump_printf("%s ", type_names[type]);
    // only print more if type is valid
    if (type_names[type] == undef)  {
    	dbg_dump_printf("descriptor hi=0x%08x, lo=0x%08x", hi, lo);
    } else {
      // for call gates, print segment:offset and parameter count p.4-15
      // for task gate, only present,dpl,TSS segment selector exist. p.5-13
      // for interrupt gate, segment:offset,p,dpl
      ...
      case BX_SYS_SEGMENT_AVAIL_386_TSS:
      case BX_SYS_SEGMENT_BUSY_386_TSS:
    	  dbg_dump_printf("at 0x%08x, length 0x%05x", base, limit);
        break;
      case BX_SYS_SEGMENT_LDT:
        // it's an LDT. not much to print.
        break;
      default:
        // task, int, trap, or call gate.
    	  dbg_dump_printf("target=0x%04x:0x%08x, DPL=%d", segment, offset, dpl);
        break;
      }
    }
    dbg_dump_printf("\n");
...
#if BX_SUPPORT_X86_64
void bx_dbg_print_descriptor64_dump(Bit32u lo1, Bit32u hi1, Bit32u lo2, Bit32u hi2)
{
  Bit32u segment = (lo1 >> 16) & 0xffff;
  Bit64u offset = (lo1 & 0xffff) | (hi1 & 0xffff0000) | ((Bit64u)(lo2) << 32);
  unsigned type = (hi1 >> 8) & 0xf;
  ...
  if (s) {
	  dbg_dump_printf("bx_dbg_print_descriptor64_dump: only system entries displayed in 64bit mode\n");
  }
  else {
    static const char *undef = "???";
    static const char *type_names[16] = {
    ...
    if (type_names[type] == undef)  {
      ...
      default:
        // task, int, trap, or call gate.
    	  dbg_dump_printf("target=0x%04x:" FMT_ADDRX ", DPL=%d", segment, offset, dpl);
        break;
      }
    }
    dbg_dump_printf("\n");
...
//FILE* bx_dbg_get_dump_log(){
//
//
//	const char *debugger_dump_log_filename = SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_DIR)->getptr();
//	  // Open debugger dump log file if needed
//	  if (strlen(debugger_dump_log_filename) > 0 && (strcmp(debugger_dump_log_filename, "-") != 0))
//	  {
//	    debugger_dump_log = fopen(debugger_dump_log_filename, "w");
//	    if (debugger_dump_log == NULL) {
//	      BX_PANIC(("Can not open debugger dump log file '%s'", debugger_dump_log_filename));
//	    }
//	    else {
//	      BX_INFO(("Using debugger dump log file %s", debugger_dump_log_filename));
//	    }
//	  }
//
//    return debugger_dump_log;
//} //2016-02-15 Ken



const char* bx_dbg_get_dump_log_dir(void){


	const char *debugger_dump_log_dir = SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_DIR)->getptr();
	//fprintf(stderr,"debugger_dump_log_dir %s\n",debugger_dump_log_dir);
    return debugger_dump_log_dir;
} //2016-02-18 Ken


void* bx_dbg_get_dump_log_info(char *dump_instr_num,char *dump_file_name,char *dump_file_num){


	strcpy(dump_instr_num, SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_INSTR_NUM)->getptr());
	strcpy(dump_file_name, SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_FILE_NAME)->getptr());
	strcpy(dump_file_num, SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_FILE_NUM)->getptr());
...
FILE* bx_dbg_set_dump_log_handler(char * debugger_dump_log_filename){
...
void bx_dbg_set_dump_events(void){
...
void bx_dbg_io_dump(Bit32u port, unsigned size, unsigned op, Bit32u val)
{

	  dbg_dump_printf("event at INSTR=" FMT_LL "d IO addr=0x%x size=%u op=%s val=0x%x\n",
      bx_pc_system.time_ticks()+1,
      port,
      size,
      (op==BX_READ) ? "read" : "write",
      (unsigned) val);
...
void bx_dbg_dma_dump(bx_phy_address addr, unsigned len, unsigned what, Bit32u val)
{
  if (bx_dbg_batch_dma.this_many == 0) {
	  dbg_dump_printf("%s: DMA batch this_many=0.\n", argv0);
    bx_dbg_exit(1);
  }

  // if Q is full, post events (and flush)
  if (bx_dbg_batch_dma.Qsize >= bx_dbg_batch_dma.this_many) {
	  dbg_dump_printf("%s: DMA batch Q was not flushed.\n", argv0);
    bx_dbg_exit(1);
  }

  // if Q already has MAX elements in it
  if (bx_dbg_batch_dma.Qsize >= BX_BATCH_DMA_BUFSIZE) {
	  dbg_dump_printf("%s: DMA batch buffer overrun.\n", argv0);
    bx_dbg_exit(1);
  }
  ...
  // if Q is full, post events (and flush)
  if (bx_dbg_batch_dma.Qsize >= bx_dbg_batch_dma.this_many)
    bx_dbg_post_dma_dump();
}

void bx_dbg_post_dma_dump(void)
{
  unsigned i;
  unsigned addr, len, what, val;
  unsigned last_addr, last_len, last_what;
    ...
    for (i=0; i<bx_dbg_batch_dma.Qsize; i++) {
      ...
      if (print_header) {
        if (!first_iteration) // need return from previous line
          ...
          first_iteration = 0;
        // need to output the event header
        dbg_dump_printf("event at INSTR=" FMT_LL "d DMA addr=0x%x size=%u op=%s val=0x%x",
                         bx_pc_system.time_ticks()+1,
                         addr, len, (what==BX_READ) ? "read" : "write", val);
        print_header = 0;
      }
...
void bx_dbg_iac_dump(unsigned vector, unsigned irq)
{
  if (bx_guard.dump.irq) {
	  dbg_dump_printf("event at INSTR=" FMT_LL "d IRQ irq=%u vec=%x\n",
      bx_pc_system.time_ticks()+1, irq, vector);
  }
}

void bx_dbg_a20_dump(unsigned val)
{
  if (bx_guard.dump.a20) {
	  dbg_dump_printf("event at INSTR=" FMT_LL "d A20 val=%u\n",
      bx_pc_system.time_ticks()+1, val);
  }
}

void bx_dbg_fetch(const Bit8u *opcode,unsigned offset,char *opcode_c){

	sprintf(opcode_c,"%02x",opcode[offset]);

}



void bx_dbg_check_mini_dump(const Bit8u *opcode, unsigned length, unsigned long long instr_num, int* reload_flag_ptr,int* smm_exit_flag_ptr){
	bx_address reg;
	unsigned int offset = 0;
	char opcode_c[16][2];
	for(int i=0;i<16;i++){
		memset(opcode_c[i],0,2);
	}
	bx_dbg_fetch(opcode,offset,opcode_c[offset]);
	//fprintf(stderr,"2222222222\n");
	while(1){
	if (!strcmp((const char *)opcode_c[offset],"f3")||!strcmp((const char *)opcode_c[offset],"66")\
			||!strcmp((const char *)opcode_c[offset],"67")){
	...
	else if(!strcmp((const char *)opcode_c[offset],"ec")|| !strcmp((const char *)opcode_c[offset],"ed")){
		dbg_dump_printf("event at INSTR=%d IO READ mini dump: ", instr_num);// IO READ instr
		#if BX_SUPPORT_SMP
			dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
		#endif
		#if BX_SUPPORT_X86_64 == 0
			reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EAX);
			dbg_dump_printf("eax(io_data): %08x ", (unsigned) reg);
			reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EDX);
			dbg_dump_printf("edx(io_port): %08x\n", (unsigned) reg);
		#else
			reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RAX);
			dbg_dump_printf("rax(io_data): %08x ",GET32L(reg));
			reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RDX);
			dbg_dump_printf("rdx(io_port): %08x\n",GET32L(reg));
	    #endif
			dbg_dump_printf("mini dump end!\n");
		break;
		}

	else if(!strcmp((const char *)opcode_c[offset],"e4")||!strcmp((const char *)opcode_c[offset],"e5")){
		offset++;
		bx_dbg_fetch(opcode,offset,opcode_c[offset]);
		dbg_dump_printf("event at INSTR=%d IO READ mini dump: ", instr_num);// IO READ instr
		#if BX_SUPPORT_SMP
			...
			dbg_dump_printf("rax(io_data): %08x ",GET32L(reg));
		#endif
			dbg_dump_printf("(io_port): 000000%s \n", opcode_c[offset]);
			dbg_dump_printf("mini dump end!\n");
		break;
		}
	else if(!strcmp((const char *)opcode_c[offset],"0f")){
		offset++;
		bx_dbg_fetch(opcode,offset,opcode_c[offset]);
		if (!strcmp((const char *)opcode_c[offset],"32")){
			dbg_dump_printf("event at INSTR=%d RDMSR mini dump: ", instr_num);// RDMSR instr
			#if BX_SUPPORT_SMP
				dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
			#endif
			#if BX_SUPPORT_X86_64 == 0
				reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EAX);
				dbg_dump_printf("eax: 0x%08x %d ", (unsigned) reg, (int) reg);
				reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EBX);
				dbg_dump_printf("ebx: 0x%08x %d ", (unsigned) reg, (int) reg);
				reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_ECX);
				dbg_dump_printf("ecx: 0x%08x %d ", (unsigned) reg, (int) reg);
				reg = BX_CPU(dbg_cpu)->get_reg32(BX_32BIT_REG_EDX);
				dbg_dump_printf("edx: 0x%08x %d ", (unsigned) reg, (int) reg);
				reg = bx_dbg_get_eip();
				dbg_dump_printf("eip: 0x%08x\n", (unsigned) reg);
			#else
				reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RAX);
				dbg_dump_printf("rax: %08x_%08x ", GET32H(reg), GET32L(reg));
				reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RBX);
				dbg_dump_printf("rbx: %08x_%08x ", GET32H(reg), GET32L(reg));
				reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RCX);
				dbg_dump_printf("rcx: %08x_%08x ", GET32H(reg), GET32L(reg));
				reg = BX_CPU(dbg_cpu)->get_reg64(BX_64BIT_REG_RDX);
				dbg_dump_printf("rdx: %08x_%08x ", GET32H(reg), GET32L(reg));
				reg = bx_dbg_get_instruction_pointer();
				dbg_dump_printf("rip: %08x_%08x\n", GET32H(reg), GET32L(reg));
		...
		else if(!strcmp((const char *)opcode_c[offset],"31")){
			dbg_dump_printf("event at INSTR=%d RDTSC mini dump: ", instr_num);// RDTSC instr
			#if BX_SUPPORT_SMP
				dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
			#endif
			#if BX_SUPPORT_X86_64 == 0
		...
		else if(!strcmp((const char *)opcode_c[offset],"33")){
			dbg_dump_printf("event at INSTR=%d RDPMC mini dump: ", instr_num);// RDPMC instr
			#if BX_SUPPORT_SMP
				dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
			#endif
			#if BX_SUPPORT_X86_64 == 0
		...
		else if(!strcmp((const char *)opcode_c[offset],"a2")){
			dbg_dump_printf("event at INSTR=%d CPUID mini dump: ", instr_num);// CPUID instr
			#if BX_SUPPORT_SMP
				dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
			#endif
			#if BX_SUPPORT_X86_64 == 0
		...
		else if(!strcmp((const char *)opcode_c[offset],"aa")){
			*smm_exit_flag_ptr = 1;
			#if BX_SUPPORT_SMP
				dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
			#endif
...
void bx_dbg_set_smm_flag(int flag){


	smm_flag = flag;
}

int bx_dbg_get_smm_flag(void){


	return smm_flag;
}

bx_address bx_dbg_get_rip(unsigned cpu){
	bx_address reg;
	reg = bx_dbg_get_instruction_pointer();
	//dbg_dump_printf("rip: %08x_%08x\n", GET32H(reg), GET32L(reg));
	return reg;
...
//void bx_dbg_dump_rip(unsigned cpu){
//	bx_address reg;
//	reg = bx_dbg_get_instruction_pointer();
//	dbg_dump_printf("rip: %08x_%08x\n", GET32H(reg), GET32L(reg));
//
...
//		char opcode0[2];
//		char opcode1[2];
//		sprintf(opcode0,"%02x",opcode[0]);
//		sprintf(opcode1,"%02x",opcode[1]);
//		if(!strcmp(opcode0,"0f") & !strcmp(opcode1,"32")){
//			dbg_dump_printf("event at INSTR=%d RDMSR mini dump: ", instr_num);// RDMSR instr
//		#if BX_SUPPORT_SMP
//			dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//		#endif
...
//		}
//
//		else if(!strcmp(opcode0,"0f") & !strcmp(opcode1,"31")){
//			dbg_dump_printf("event at INSTR=%d RDTSC mini dump: ", instr_num);// RDTSC instr
//		#if BX_SUPPORT_SMP
//			dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//		#endif
...
//				dbg_dump_printf("mini dump end!\n");
//			}
//		else if(!strcmp(opcode0,"e4")||!strcmp(opcode0,"e5")){
//			dbg_dump_printf("event at INSTR=%d IO READ mini dump: ", instr_num);// IO READ instr
//					#if BX_SUPPORT_SMP
//						dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//					#endif
...
//		}
//
//		else if(!strcmp(opcode0,"0f")||!strcmp(opcode0,"33")){
//				dbg_dump_printf("event at INSTR=%d RDPMC mini dump: ", instr_num);// RDPMC instr
//						#if BX_SUPPORT_SMP
//							dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//						#endif
...
//		sprintf(opcode0,"%02x",opcode[0]);
//		sprintf(opcode1,"%02x",opcode[1]);
//		sprintf(opcode2,"%02x",opcode[1]);
//		if(!strcmp(opcode0,"0f") & !strcmp(opcode1,"01") & !strcmp(opcode1,"f9")){
//					dbg_dump_printf("event at INSTR=%d RDTSC mini dump: ", instr_num);// RDTSCP instr
//				#if BX_SUPPORT_SMP
//					dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//				#endif
//				#if BX_SUPPORT_X86_64 == 0
...
//		else if(!strcmp(opcode0,"0f") & !strcmp(opcode1,"07") & !strcmp(opcode1,"f9")){
//					dbg_dump_printf("event at INSTR=%d\tRDTSC mini dump:\n", instr_num);// RDRAND instr
//				#if BX_SUPPORT_SMP
//					dbg_dump_printf("CPU%d:\n", BX_CPU(dbg_cpu)->bx_cpuid);
//				#endif
//				#if BX_SUPPORT_X86_64 == 0
