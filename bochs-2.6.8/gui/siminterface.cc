...
public:
  ...
  virtual int set_debugger_dump_log_file(const char *path); //2016-02-15 Ken
...
static bx_param_c *find_param(const char *full_pname, const char *rest_of_pname, bx_param_c *base)
{
  ...
  *to = 0;
  ...
  //BX_ERROR(("searching for component '%s' in list '%s'", component, base->get_name())); Ken 2016-02-15 for debug
...
bx_param_c *bx_real_sim_c::get_param(const char *pname, bx_param_c *base)
{
  if (base == NULL)
    base = root_param;
  //BX_ERROR(("pname is %s and base is %s", pname, base->get_name())); Ken 2016-02-15 for debug
...
int bx_real_sim_c::set_debugger_dump_log_file(const char *path)
{
  SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_DIR)->set(path);
  return 0;
} //Ken 2016-02-15
