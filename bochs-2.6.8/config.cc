...
//####################### Ken added initial parameter###########################
  path = new bx_param_filename_c(menu,
      "debugger_dump_dir",
      "Debugger Dump Log dir",
      "dirname of debugger dump log file",
      "-", BX_PATHNAME_LEN);
  path->set_ask_format("Set debugger dump log file dir name: [%s] ");
  path->set_extension("log");
  path->set_enabled(BX_DEBUGGER);
  path = new bx_param_filename_c(menu,
      "dump_instr_num",
      "Debugger dump_instr num",
      "Instr num of debugger dump log file",
      "-", BX_PATHNAME_LEN);
  path->set_ask_format("Set debugger dump log file instr num: [%s] ");
  path->set_extension("log");
  path->set_enabled(BX_DEBUGGER);
  path = new bx_param_filename_c(menu,
      "dump_file_name",
      "Debugger Dump Log filename",
      "The name of debugger dump log file",
      "-", BX_PATHNAME_LEN);
  path->set_ask_format("Set debugger dump log file name: [%s] ");
  path->set_extension("log");
  path->set_enabled(BX_DEBUGGER);
  path = new bx_param_filename_c(menu,
      "dump_file_num",
      "Debugger Dump Log num",
      "The num of debugger dump log file",
      "-", BX_PATHNAME_LEN);
  path->set_ask_format("Set debugger dump log file num: [%s] ");
  path->set_extension("log");
  path->set_enabled(BX_DEBUGGER); //Ken 2016-02-15
//####################### Ken added ##########################################
...
static int parse_line_formatted(const char *context, int num_params, char *params[])
{
  ...
  }else if (!strcmp(params[0], "debugger_dump_dir")) {
      if (num_params != 2) {
        PARSE_ERR(("%s: debugger_dump_log directive has wrong # args.", context));
      }
      SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_DIR)->set(params[1]);
  } //2016-02-15 Ken
  else if (!strcmp(params[0], "dump_instr_num")) {
        if (num_params != 2) {
          PARSE_ERR(("%s: dump_instr_num directive has wrong # args.", context));
        }
        SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_INSTR_NUM)->set(params[1]);
    } //2016-02-15 Ken
  else if (!strcmp(params[0], "dump_file_name")) {
        if (num_params != 2) {
          PARSE_ERR(("%s: dump_file_name directive has wrong # args.", context));
        }
        SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_FILE_NAME)->set(params[1]);
    } //2016-02-15 Ken
  else if (!strcmp(params[0], "dump_file_num")) {
        if (num_params != 2) {
          PARSE_ERR(("%s: dump_file_num directive has wrong # args.", context));
        }
        SIM->get_param_string(BXPN_DEBUGGER_DUMP_LOG_FILE_NUM)->set(params[1]);
    } //2016-02-15 Ken
